...
- [x] Migration `000002`: add nullable profile columns (`current_location`, `current_role`, `current_company`, `current_salary`, `salary_currency`) to users table
- [x] Update `User` entity + `UserModel` with new fields
- [x] Add `Update()` to `UserRepository` interface + `PostgresUserRepo` implementation
- [x] Add `UpdateProfileRequest` DTO with `Validate()` (name non-empty, currency non-empty)
- [x] New `UpdateProfileUseCase` with unit test (uses `mock_user_repository`)
- [x] Extend `AuthHandler`: new `UpdateMe` handler, update `Me` to return full `UserInfo`
- [x] Router: add `r.Put("/me", authHandler.UpdateMe)` in protected auth group
- [x] Wire in `main.go`
- [x] `make mock && make swagger && make lint && make test`
- [x] PR + auto-merge
...
### [x] PR-28 — feat(user): user profile page [frontend]
...
- [x] Extend `User` interface + add `api.auth.updateProfile()` in `api.ts`
- [x] `AuthContext`: add `updateUser()`, fix page-refresh hydration with `useEffect`
- [x] New `ProfilePage.tsx`: `useQuery` + `useMutation`, 3 states, dark mode, `motion/react`
- [x] `Navbar.tsx`: replace logout button with user dropdown (Profile / Logout), click-outside close
- [x] `App.tsx`: add `/profile` route under `ProtectedLayout`
- [x] i18n: add `profile.*` + `nav.profile` / `nav.logout` keys to `en.json` + `vi.json`
- [x] Smoke test: navigate to `/profile`, edit name, save, assert toast
- [x] `make lint && make test-ui`
- [x] PR + auto-merge
